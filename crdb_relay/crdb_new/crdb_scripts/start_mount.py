...
def execute_ssh_command(host):
    # 定义SSH命令
    ssh_command = (
        #"cd share/crdb_new/crdb_scripts && sudo chmod +x bazelisk-linux-amd64 && sudo cp bazelisk-linux-amd64 /usr/local/bin/bazel && sudo ln -s /usr/local/bin/bazel/bazelisk-linux-amd64 /usr/bin/bazel && cd /home/ubuntu/share/crdb_rls && bazel version"
        "sudo mount -t nfs 103.4.15.248:/home/ubuntu/share share"
    )
    
    # 打印当前主机和要执行的命令
    print(f"Executing on {host}: {ssh_command}")
...
with concurrent.futures.ThreadPoolExecutor(max_workers=len(hosts)) as executor:
    # 为每个主机提交执行任务
    futures = [executor.submit(execute_ssh_command, host) for host in hosts]
    
    # 等待所有任务完成（可选）
    for future in concurrent.futures.as_completed(futures):
        future.result()  # 这里可以处理每个任务的结果或异常
...
for i in range(len(hosts)):
    ...
    #ssh_command = "sudo mkdir share ; sudo chmod 777 share ; sudo mount -t nfs 103.4.15.248:/home/ubuntu/share share"
    #wget https://binaries.cockroachdb.com/cockroach-v23.2.2.linux-amd64.tgz | tar -xz
    ssh_command = "sudo docker swarm join --token SWMTKN-1-3ihpt3grpbd5h579sxfpsmhtguy50gpromictun0hg9fdptv8s-ce1q5sn81b7e5nxe3z7jyrktf 103.4.15.248:2377"
    #ssh_command = "sudo mkfs -t xfs /dev/nvme1n1 && sudo mount /dev/nvme1n1 /home/ubuntu/data && sudo chmod -R 777 /home/ubuntu/data"
    #ssh_command = "sudo mkdir tem ; sudo mv /usr/local/bin/cockroach tem"
    print(ssh_command)
...
" rjgong/crdb cd crdb && sudo chmod +x bazelisk-linux-amd64 && sudo cp bazelisk-linux-amd64 /usr/local/bin/bazel && sudo ln -s /usr/local/bin/bazel/bazelisk-linux-amd64 /usr/bin/bazel && ./crdb/cockroach/cockroach start \
